<template>
  <div class="nav-wrapper" v-show="isMenuOpen">
    <nav class="nav">
      <div class="nav--city">
        <Select
          class="select--cities"
      ...
      <div class="nav--inner">
        ...
        <router-link
          @click.native="$store.commit('menuHide')"
          class="nav--item nav--item-certificate"
          to="/certificate"
          >Сертификация</router-link
        >
        <a
          href="/"
          @click.prevent="navClickhandle('Proposal')"
          class="nav--item"
          to="/proposal"
          >Добавить студию</a
        >
...
export default {
  ...
  computed: {
    cities() {
      return this.$store.state.cities.unique_cities;
    },
    city() {
    ...
    selectOptionHandle(data) {
      switch (data) {
        ...
        case "Индивидуальный подбор студии":
          this.$store.dispatch("updateModal", { name: "SpecSelection" });
          break;
