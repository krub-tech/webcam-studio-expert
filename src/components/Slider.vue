<template>
    <div class="slider">
        ...
        <div class="points">
            <span v-for="point in data.length" :key="point" class="point" :class="{ active: point - 1 == count }" />
        </div>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    move() {
      this.$el.querySelector('.slides').style.left = `-${this.count * this.sliderWidth}px`;
    },
    optimizeWidth() {
