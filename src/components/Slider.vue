<template>
  <div class="slider">
    <div class="slides">
      <div class="slide" v-for="slide in data" :key="slide">
        <img :src="slide" />
      </div>
    </div>
...
export default {
  ...
  methods: {
    prev() {
      this.count === 0 ? (this.count = this.data.length) : false
      this.count--
      this.move()
    },
    next() {
      this.count++
      this.count === this.data.length ? (this.count = 0) : false
      this.move()
    },
    move() {
      this.$el.querySelector('.slides').style.left = `-${this.count *
        this.sliderWidth}px`
    },
    optimizeWidth() {
