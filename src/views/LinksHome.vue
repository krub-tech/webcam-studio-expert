...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('getLinksFromDB', `?category=${this.currentCategory}`);
  },
  beforeUpdate() {},
  methods: {
    isCurrent(str) {
      if (this.$store.state.windowWidth < 420) return false;
      return this.currentCategory === str;
    },
    linkCategoryClickHandle(category) {
      this.$store.commit('loadCurrentLinksCategory', category);
      this.$store.dispatch('getLinksFromDB', `?category=${category}`);
      if (this.$store.state.windowWidth < 420) this.$router.push(`/links/${this.currentCategory}`);
    },
