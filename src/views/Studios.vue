<template>
  <section class="studios">
    ...
    <div class="studios--title">
      <h1>Вебкам студии г. {{ $store.state.cities.currentCity }}</h1>
    ...
    <div class="studios--count" :class="{ dirty: isFiltred }">
      <p class="studios--count-current">{{ currentStudiosLength }}</p>
      /{{ allStudiosLength }}
    </div>
...
export default {
  ...
  data() {
    return {
      for_sort: ["По умолчанию", "По названию", "По процентам"],
      filterMobShow: false,
    };
  },
  components: {
  ...
  computed: {
    currentStudiosLength() {
      return this.$store.state.studios.currentStudiosLength;
    },
    allStudiosLength() {
      return this.$store.state.studios.allStudiosLength;
    },
    isFiltred() {
      return this.currentStudiosLength !== this.allStudiosLength;
    },
  },
  methods: {
